...
class PostPagesTests(TestCase):
    ...
    def test_index_show_correct_context(self):
        ...
        expected = list(Post.objects.all()[:QUANTITY_POST])
    ...
    def test_group_list_show_correct_context(self):
        ...
        expected = list(
            Post.objects.filter(
                group_id=self.group.id)[:QUANTITY_POST])
        self.assertEqual(list(response.context["page_obj"]), expected)
    ...
    def test_profile_show_correct_context(self):
        ...
        expected = list(
            Post.objects.filter(
                author_id=self.user.id)[:QUANTITY_POST])
        self.assertEqual(list(response.context["page_obj"]), expected)
    ...
    def test_check_group_in_pages(self):
        ...
        form_fields = {
            reverse(
                "posts:posts_index"): Post.objects.get(group=post.group),
            reverse(
                "posts:group_list", kwargs={"slug": group.slug}
            ): Post.objects.get(group=post.group),
            reverse(
    ...
    def test_check_cache(self):
        ...
        response2 = self.client.get(reverse("posts:posts_index"))
